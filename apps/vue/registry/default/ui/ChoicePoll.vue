...
interface ChoicePollOptionContext {
  optionId: string
  disabled: boolean
}

const CHOICE_POLL_KEY: InjectionKey<ChoicePollContext> = Symbol('choice-poll')
const CHOICE_POLL_OPTION_KEY: InjectionKey<ChoicePollOptionContext> = Symbol('choice-poll-option')

export function useChoicePoll() {
  const ctx = inject(CHOICE_POLL_KEY)
  if (!ctx) throw new Error('ChoicePoll components must be used within ChoicePollRoot')
  return ctx
}

export function useChoicePollOption() {
  const ctx = inject(CHOICE_POLL_OPTION_KEY)
  if (!ctx) throw new Error('ChoicePoll.Option sub-components must be used within ChoicePoll.Option')
  return ctx
}
...
<template>
  <div
    role="group"
    :aria-label="multiple ? 'Multiple choice poll' : 'Single choice poll'"
