...
function CatalogOutboundChip({
  ...
  return (
    <a
      className={cn(
        "bg-background inline-flex w-fit items-center gap-2 rounded-md border px-3 py-2 font-mono text-[10px] tracking-widest uppercase antialiased transition-colors duration-200",
        tone === "ai" &&
  ...
  return (
    ...
    >
      <ProductCatalogRegionHeader
        headerClassName={cn(
          isAi ? "border-b bg-white/80" : "border-b bg-white/80",
          // isAi
        ...
        <div className="mx-auto max-w-6xl px-4 py-5 md:px-6">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between md:gap-8">
            <div className="flex gap-4">
              <div
                className={cn(
                  "border-border bg-background flex size-10 shrink-0 items-center justify-center rounded-lg border",
                  isAi &&
              ...
              <div className="min-w-0 space-y-1">
                <p
                  className={cn(
                    "font-mono text-[10px] tracking-[0.2em] uppercase antialiased",
                    isAi
                      ? "text-foreground"
                ...
                <h2
                  className={cn(
                    "text-base font-medium tracking-tight text-balance antialiased md:text-lg",
            ...
            <div className="hidden flex-row items-center gap-3 md:flex md:flex-col md:items-end md:pt-1">
              <CatalogOutboundChip
                href={
...
function ProductPromoCard({
  ...
  return (
    <BackgroundImageTexture
      className="shadow-elevation-light h-full w-full overflow-hidden rounded-xl sm:max-w-sm"
      opacity={isAi ? 0.99 : 0.29}
      variant={isAi ? "debut-light" : "inflicted"}
    >
      <article
        className={cn(
          "group relative flex h-full w-full flex-col rounded-xl p-6 transition-all duration-200 sm:max-w-sm md:p-8",
          isAi
            ? "bg-background/10 border-[#006BFF]/30"
            : "bg-background/10 border-[#FF6BFF]/35",
          SURFACE_SHADOW,
          SURFACE_SHADOW_DARK
        )}
      >
        <div className="mb-5 w-fit">{badge}</div>
        <p className="text-foreground/75 mb-8 flex-1 text-sm leading-relaxed text-pretty md:text-base">
          {description}
        </p>
        <div className="mt-auto flex flex-col gap-3">
          <a
            className={cn(
              "text-background inline-flex w-full items-center justify-center gap-2 rounded-md px-4 py-3 font-mono text-xs tracking-[0.14em] uppercase transition-colors duration-200 md:w-auto md:self-start",
              isAi
...
export function HomeBelowHeroLayoutAlternate() {
  return (
    <section className="w-full md:block">
      <div className="mx-auto w-full max-w-4xl px-4 pt-4 pb-8 md:pt-2">
        <LatestComponentVertical />
      </div>
      ...
      <BackgroundImageTexture
        ...
        variant="groovepaper"
      >
        <div className="border-border/50 bg-muted/10 border-t py-10 md:py-12">
            ...
            >
              <div className="mb-10 text-center md:mb-12">
                <div className="mx-auto mb-8 flex max-w-2xl items-center justify-center gap-4">
                  <div className="to-border h-px flex-1 bg-linear-to-r from-transparent" />
                  <span className="text-muted-foreground shrink-0 font-mono text-[10px] tracking-[0.2em] uppercase">
                    Commercial products
                  </span>
                  <div className="to-border h-px flex-1 bg-linear-to-l from-transparent" />
                </div>
                <h2 className="font-pixel-square mx-auto mb-4 max-w-2xl text-2xl font-medium tracking-tight text-balance md:text-3xl">
                  Support Cult UI open source
                </h2>
                <p className="font-pixel-square text-foreground/70 mx-auto max-w-lg text-base leading-relaxed text-pretty md:max-w-xl md:text-lg">
              ...
              <div className="flex flex-col items-center justify-center gap-5 md:flex-row">
                <ProductPromoCard
                  badge={
                    <div
                      className={cn(
                        "bg-background/10 flex items-center gap-2 rounded-md px-3 py-2",
                        SURFACE_SHADOW,
                        SURFACE_SHADOW_DARK
                      )}
                    >
                ...
                <ProductPromoCard
                  badge={
                    <div
                      className={cn(
                        "bg-background/20 flex items-center gap-2 rounded-md px-3 py-2",
                        SURFACE_SHADOW,
                        SURFACE_SHADOW_DARK
                      )}
                    >
      ...
      <div className="bg-muted/40 border-border/50 border-t border-b">
        <div className="mx-auto max-w-4xl px-12 sm:px-2">
          <PlugCardGrid />
        </div>
      </div>
    </section>
